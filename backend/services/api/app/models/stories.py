...
class StoryHighlight(Base):
    """مجموعهٔ ماندگارِ داستان‌ها روی پروفایل (مثلِ Highlights اینستاگرام).

    برخلافِ Story که ۲۴ساعته منقضی می‌شود، هایلایت دائمی است. هر آیتم یک
    اسنپ‌شات از رسانهٔ داستان نگه می‌دارد تا با انقضا/حذفِ داستانِ اصلی از بین نرود.
    """
    ...
    title      = Column(String(60), nullable=False, default="هایلایت")
    ...
    __table_args__ = (
        Index("ix_highlight_owner", "owner_id", "updated_at"),
    )


class StoryHighlightItem(Base):
    """یک داستانِ ذخیره‌شده درونِ یک هایلایت (اسنپ‌شاتِ مستقل از داستانِ اصلی)."""
    __tablename__ = "story_highlight_items"

    id             = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    highlight_id   = Column(
        UUID(as_uuid=True),
        ForeignKey("story_highlights.id", ondelete="CASCADE"),
        nullable=False, index=True,
    )
    story_id       = Column(UUID(as_uuid=True), nullable=True)  # provenance (بدونِ FK)
    media_url      = Column(Text, nullable=False)
    media_type     = Column(String(12), nullable=False, default="image")  # image | video
    caption        = Column(Text, nullable=True)
    ...
    __table_args__ = (
        Index("ix_highlight_item_hl", "highlight_id", "sort_order"),
    )
