...
class MessageRoom(Base):
    ...
    disappear_seconds = Column(Integer, nullable=False, default=0)  # پیامِ ناپدیدشونده: TTL بر حسبِ ثانیه (0 = خاموش)
...
class Message(Base):
    ...
    expires_at  = Column(DateTime(timezone=True), nullable=True, index=True)  # پیامِ ناپدیدشونده: پس از این زمان از نمایش حذف می‌شود
    created_at  = Column(DateTime(timezone=True), nullable=False, default=_now)
...
class MessageEvent(Base):
    """رویدادِ پیوست‌شده به یک پیام (media_type='event'): عنوان، زمانِ شروع، مکان و توضیح."""
    ...
    __table_args__ = (
        Index("uq_event_message", "message_id", unique=True),
    )


class UserReport(Base):
    """گزارشِ تخلفِ کاربر/پیام: reporter کاربرِ reported را (اختیاراً دربارهٔ یک پیام) گزارش کرده."""
    ...
    reason      = Column(String(40), nullable=False)   # spam | harassment | scam | inappropriate | other
    note        = Column(Text, nullable=True)
    status      = Column(String(16), nullable=False, default="open")  # open | reviewed | dismissed
    created_at  = Column(DateTime(timezone=True), nullable=False, default=_now)


class MessageRedPacket(Base):
    """🧧 هدیهٔ نقدی (Red Packet) پیوست‌شده به یک پیام (media_type='red_packet').

    فرستنده مبلغِ کل را از کیف‌پول خرج می‌کند و به تعدادِ مشخصی «سهم» تقسیم می‌شود؛
    اعضای اتاق هرکدام یک‌بار «باز» می‌کنند و سهمی می‌گیرند (مساوی یا شانسی/lucky).
    سهم‌های باز‌نشده پس از انقضا به فرستنده بازگردانده می‌شود.
    """
    ...
    total_amount   = Column(Integer, nullable=False)        # مبلغِ کل به ریال
    count          = Column(Integer, nullable=False)        # تعدادِ سهم‌ها
    ...
    mode           = Column(String(8), nullable=False, default="equal")   # equal | random
    greeting       = Column(String(200), nullable=True)
    status         = Column(String(12), nullable=False, default="active") # active | finished | refunded
    expires_at     = Column(DateTime(timezone=True), nullable=False)
    created_at     = Column(DateTime(timezone=True), nullable=False, default=_now)

    __table_args__ = (
        Index("uq_red_packet_message", "message_id", unique=True),
        Index("ix_red_packet_status", "status", "expires_at"),
    )


class RedPacketClaim(Base):
    """سهمِ برداشت‌شده از یک هدیهٔ نقدی توسطِ یک کاربر (یکتا به‌ازای packet+user)."""
    __tablename__ = "red_packet_claims"

    id         = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    packet_id  = Column(UUID(as_uuid=True), ForeignKey("red_packets.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id    = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
