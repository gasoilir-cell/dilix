...
def test_provider_sandbox_result_schema() -> None:
    ...
    r = SandboxTestResult(
        api_id=uuid.uuid4(), reachable=True, http_status=200, latency_ms=42, detail="ok"
    )
...
async def test_webrtc_relay_forwards_to_target() -> None:
    ...
    try:
        await rt._relay_signal(WsEventType.CALL_OFFER, "caller-1", {"to": "callee-2", "sdp": "v=0"})
    finally:
