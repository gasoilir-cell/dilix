...
## [2026-07-18] تستِ ویجت برای ۷ صفحهٔ فاقدِ پوشش (dashboard/wallet/insurance/telecom + messages/discovery/investment)

هفت صفحهٔ موبایلِ بدونِ تستِ ویجت پوشش داده شد (smoke render + assertِ برچسب‌های فارسیِ واقعی از خودِ فایلِ screen).

**تست‌ها (`frontend/mobile/test`):** الگوی `services_screen_test.dart` (همان import/`_fakeApi`/`_wrap`).
- `dashboard_screen_test.dart`: `MockClient` مسیرِ `/v1/identity/me` را هویت و `/v1/growth/rewards` را کیفِ خالی می‌دهد؛ assert: «نمای کلی» + «خدماتِ من» + «کیفِ پول» + «کرهٔ زمین» + «همهٔ خدمات» (`pumpAndSettle`).
- `wallet_screen_test.dart`: کیفِ پاداشِ IRR؛ assert: «کیف پول» + «موجودیِ پاداش» + «انتقالِ امن» + «لینکِ دعوت».
- `insurance_screen_test.dart`: بدونِ درخواستِ اولیه؛ assert: «بیمه» + «استعلامِ بیمه» + دکمهٔ «استعلام».
- `telecom_screen_test.dart`: assert: «ارتباطات» + تب‌های «شارژ / اینترنت»/«eSIM» + دکمهٔ «ثبتِ شارژ».
- `messages_screen_test.dart`: کلاینتِ بدونِ توکن → پرامپتِ ورود؛ assert: «پیام‌ها» + `textContaining('گفتگوهای رمزنگاری‌شده')`.
- `discovery_screen_test.dart`: assert: «کشفِ اطراف» + `textContaining('یافتنِ افراد و کسب‌وکارهای نزدیک')` + دکمهٔ «جستجو».
- `investment_screen_test.dart`: `investmentPositions` را `[]` می‌دهد؛ assert: «سرمایه‌گذاری» + «خریدِ واحدِ صندوق» + «موقعیت‌های من» + دکمهٔ «خرید» (`pumpAndSettle`).

**اعتبارسنجی:** موازنهٔ ()/[]/{} برای هر ۷ فایل سبز. متن‌های فارسی مستقیماً از فایلِ `*_screen.dart` برداشته شد (اختراع‌نشده). گام‌های CI در `mobile.yml` تأیید شد: `flutter analyze || true` (خط ۱۱۵)، `flutter test` (خط ۱۲۱، همهٔ `test/*.dart` را glob می‌کند → هر ۷ فایلِ جدید خودکار پوشش داده می‌شوند)، `flutter build apk --release` (خط ۱۲۵). `flutter` در کانتینر نیست؛ analyze/test به CI موکول.
