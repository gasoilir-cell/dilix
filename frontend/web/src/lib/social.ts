// Dilix — دریافتِ توکنِ ورودِ اجتماعی در مرورگر.
// هر تابع SDK مربوطه را به‌صورتِ تنبل (lazy) بارگذاری می‌کند و توکنی برمی‌گرداند
// که سپس به بک‌اند (POST /api/v1/auth/oauth/{provider}) فرستاده می‌شود.
...
const env = (key: string): string => ENV_MAP[key] ?? "";
...
function loadScript(src: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => reject(new SocialAuthError("بارگذاریِ اسکریپت ناموفق")));
      return;
    }
    ...
    s.addEventListener("error", () => reject(new SocialAuthError("بارگذاریِ اسکریپت ناموفق")));
...
function requireEnv(key: string, label: string): string {
  const v = env(key);
  if (!v) throw new SocialAuthError(`${label} هنوز فعال نشده است (کلید پیکربندی نشده).`);
  return v;
}
...
export async function getMicrosoftIdToken(): Promise<string> {
  ...
  const authority =
    env("NEXT_PUBLIC_MICROSOFT_AUTHORITY") || "https://login.microsoftonline.com/common";
