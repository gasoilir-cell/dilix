...
const ROLES = [
  ...
  { id: "driver",          icon: Truck,     color: "#F97316", bg: "rgba(249,115,22,0.15)" },
  { id: "cargo_owner",     icon: Package,   color: "#06B6D4", bg: "rgba(6,182,212,0.15)" },
  { id: "freight_broker",  icon: Handshake, color: "#A855F7", bg: "rgba(168,85,247,0.15)" },
  { id: "insurance_agent", icon: Shield,    color: "#10B981", bg: "rgba(16,185,129,0.15)" },
  { id: "banker",          icon: Landmark,  color: "#EAB308", bg: "rgba(234,179,8,0.15)" },
  { id: "creator",         icon: Building2, color: "#F43F5E", bg: "rgba(244,63,94,0.15)" },
] as const;

// ── Form schema ───────────────────────────────────────────────
const makeProfileSchema = (t: (k: string) => string) =>
  z.object({
    full_name: z.string().min(2, t("onb.err.nameMin")),
    username: z
      .string()
      ...
      .pipe(
        z
          .string()
          .min(3, t("onb.err.userMin"))
          .max(30, t("onb.err.userMax"))
          .regex(/^[a-z0-9_.]+$/, t("onb.err.userRegex"))
      )
      .optional()
      .or(z.literal("")),
  });
...
export default function OnboardingPage() {
  ...
  const onRoleNext = () => {
    if (!selectedRole) { toast.error(t("onb.toast.selectRole")); return; }
    setStep(2);
  };
  ...
  // ── Progress indicator ────────────────────────────────────
  const steps = [
    { n: 1, label: t("onb.step.account") },
    { n: 2, label: t("onb.step.profile") },
    { n: 3, label: t("onb.step.privacy") },
  ];

  return (
    <div
      className="fixed inset-0 overflow-y-auto px-4 py-10"
              ...
              <div key={s.n} className="flex items-center gap-2">
                <div className="flex flex-col items-center gap-1">
                  <div
                    ...
                    style={{
                      background: step > s.n ? "#10B981" : step === s.n ? "#6366F1" : "var(--border-1)",
                      color:      step >= s.n ? "#fff" : "var(--t-3)",
                      boxShadow:  step === s.n ? "0 0 0 3px rgba(99,102,241,0.3)" : "none",
                    }}
                  >
                    {step > s.n ? <Check size={14} /> : s.n}
                  </div>
                  <span className="text-[10px]" style={{ color: step >= s.n ? "var(--t-2)" : "var(--t-5)" }}>
                    {s.label}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div
                    ...
                    style={{ background: step > s.n ? "#10B981" : "var(--border-1)" }}
                  />
                )}
              </div>
            ))}
      ...
      <div className="w-full max-w-md">
        ...
        {step === 1 && (
          <div style={card}>
            <div className="text-center mb-6">
              ...
              <h1 className="text-xl font-bold text-white">{t("onb.welcome")}</h1>
              <p className="text-sm mt-1" style={{ color: "var(--t-3)" }}>
                {t("onb.welcomeSub")}
              </p>
            </div>
              ...
              {ROLES.map((r) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background:  active ? r.bg : "var(--surface-2)",
                      border:      active
                        ? `2px solid ${r.color}`
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{ color: active ? r.color : "var(--t-1)" }}
                    >
                      {t(`onb.role.${r.id}.label`)}
                    </p>
                    <p
                      className="text-[11px] mt-0.5 leading-tight"
                      style={{ color: "var(--t-3)" }}
                    >
                      {t(`onb.role.${r.id}.desc`)}
                    </p>
                  </button>
                );
              })}
            ...
            <p className="text-xs text-center mb-4" style={{ color: "var(--t-4)" }}>
              {t("onb.roleNote")}
            </p>
        ...
        {step === 2 && (
          <div style={card}>
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-white">{t("onb.profileTitle")}</h2>
              <p className="text-sm mt-1" style={{ color: "var(--t-3)" }}>
            ...
            <form onSubmit={handleSubmit(onProfileSubmit)} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--t-2)" }}>
                  {t("onb.fullName")} <span className="text-red-400">*</span>
                </label>
                <input
                  {...register("full_name")}
                  placeholder={t("onb.fullNamePh")}
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--t-2)" }}>
                  {t("onb.username")} <span style={{ color: "var(--t-4)" }}>{t("onb.optional")}</span>
                </label>
              ...
              <div className="flex gap-3 mt-6">
                ...
                >
                  <ChevronRight size={20} style={{ color: "var(--t-2)" }} />
                </button>
                <button
                  type="submit"
        ...
        {step === 3 && (
          <div style={card}>
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-white">{t("onb.privacyTitle")}</h2>
              <p className="text-sm mt-1" style={{ color: "var(--t-3)" }}>
                {t("onb.privacySub")}
              </p>
            </div>
              ...
              {[
                {
                  value: false,
                  icon: EyeOff,
                  title: t("onb.privacy.hiddenTitle"),
                  desc: t("onb.privacy.hiddenDesc"),
                  color: "#6366F1",
                },
                {
                  value: true,
                  icon: Eye,
                  title: t("onb.privacy.visibleTitle"),
                  desc: t("onb.privacy.visibleDesc"),
                  color: "#10B981",
                },
              ].map((opt) => {
                const Icon   = opt.icon;
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? `${opt.color}18` : "var(--surface-2)",
                      border:     `2px solid ${active ? opt.color : "var(--border-1)"}`,
                    }}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 text-right">
                        <p
                          ...
                          style={{ color: active ? opt.color : "var(--t-1)" }}
                        >
                          {opt.title}
                        </p>
                      ...
                      <div
                        ...
                        style={{
                          borderColor: active ? opt.color : "var(--t-5)",
            ...
            <p className="text-xs text-center mb-5" style={{ color: "var(--t-4)" }}>
              {t("onb.privacyNote")}
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                style={{ background: "linear-gradient(135deg,#6366F1,#818CF8)", boxShadow: "0 4px 16px rgba(99,102,241,0.4)" }}
              >
                {isLoading ? t("onb.saving") : t("onb.start")}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div style={{ ...card, textAlign: "center" }}>
            ...
            <h2 className="text-2xl font-bold text-white mb-2">{t("onb.doneTitle")}</h2>
            <p className="text-sm" style={{ color: "var(--t-3)" }}>
              {t("onb.redirecting")}
            </p>
          </div>
        )}
