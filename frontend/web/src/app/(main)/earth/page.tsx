...
import {
  ...
  Handshake, X, Users, ChevronRight, MapPin, Navigation, Image as ImageIcon,
} from "lucide-react";
import BottomNav from "@/components/layout/BottomNav";
import AiFloatButton from "@/components/shared/AiFloatButton";
import { useRouter, useSearchParams } from "next/navigation";
import { toPersianNum } from "@/lib/utils";
import { earthApi, postsApi } from "@/lib/api";
...
const ROLE_LABEL: Record<string, string> = {
  driver: "earth.role.driver", cargo_owner: "earth.role.cargo_owner", freight_broker: "earth.role.freight_broker",
  insurance_agent: "earth.role.insurance_agent", creator: "earth.role.creator", admin: "earth.role.admin", user: "earth.role.user",
};
...
const rl  = (r: string) => ROLE_LABEL[r] ?? "earth.role.user";
...
// «لحظه» — پستِ دارای موقعیت روی کره
interface MomentPin {
  id: string; lat: number; lng: number;
  media_url: string; media_type: string;
  caption?: string | null; place_name?: string | null;
  author_name: string; author_earth_id: string; author_avatar?: string | null;
  created_at: string;
}
...
export default function EarthPage() {
  ...
  // لحظه‌ها (پست‌های دارای موقعیت) — لایهٔ جداگانه روی همان کره
  const [moments,    setMoments]    = useState<MomentPin[]>([]);
  const [showMoments, setShowMoments] = useState(true);
  const [momentView, setMomentView] = useState<MomentPin | null>(null);
  const momentNodes = useRef<Map<string, HTMLDivElement>>(new Map());
  const momentsRef  = useRef<MomentPin[]>([]);
  ...
  // /*rt-poll*/ پولِ کاربران هر ۱۵ث تا آنلاین/آفلاین‌شدن بدونِ رفرشِ دستی روی نقشه به‌روز شود
  useEffect(() => {
    const iv = setInterval(() => { fetchUsers(); }, 15000);
    return () => clearInterval(iv);
  }, [fetchUsers]);

  // بارگذاریِ لحظه‌ها (پست‌های دارای موقعیت)
  useEffect(() => {
    postsApi.moments()
      .then((r) => setMoments((r.data ?? []).filter((m: MomentPin) => m.lat != null && m.lng != null)))
      .catch(() => {});
  }, []);
  momentsRef.current = showMoments ? moments : [];
      ...
      earthApi
        ...
        .then(() => {
          ...
          if (recenter) {
            ...
            toast.success(t("earth.toast.locSaved"));
          }
        })
        .catch(() => {
          if (recenter)
            toast.error(t("earth.toast.locErr"));
        });
    },
    [fetchUsers]
  );
  ...
  const startTracking = useCallback(
    (permanent: boolean) => {
      if (typeof navigator === "undefined" || !navigator.geolocation) {
        toast.error(t("earth.toast.noGeo"));
        return;
      }
  ...
  const updateMarkers = useCallback(() => {
    ...
    const place = (lat: number, lng: number, node: HTMLElement) => {
      const co = g.getCoords(lat, lng, MARKER_ALT);
      const sc = g.getScreenCoords(lat, lng, MARKER_ALT);
      const facing = cp
    ...
    for (const p of pointsRef.current) {
      ...
      if (node) place(p.lat, p.lng, node);
    }
    for (const m of momentsRef.current) {
      const node = momentNodes.current.get(m.id);
      if (node) place(m.lat, m.lng, node);
    }
  }, [GlobeComp]);
  ...
  }, [GlobeComp, points, moments, showMoments, updateMarkers]);
  ...
  const onGlobeReady = () => {
    ...
    // پیوندِ عمیق: /earth?focus=lat,lng → پرواز به همان نقطه (از بَجِ لوکیشنِ پست)
    const focus = searchParams.get("focus");
    let flew = false;
    if (focus) {
      const [flat, flng] = focus.split(",").map((s) => parseFloat(s));
      if (Number.isFinite(flat) && Number.isFinite(flng)) {
        controls.autoRotate = false;
        g.pointOfView({ lat: flat, lng: flng, altitude: 0.35 }, 1400);
        flew = true;
      }
    }
    if (!flew) g.pointOfView({ lat: 32, lng: 53, altitude: 2.2 }, 0);
    requestAnimationFrame(updateMarkers);
  };

  // ── JSX ───────────────────────────────────────────────────────────────────
  return (
    <div className="relative overflow-hidden" style={{ height: "calc(100dvh - 64px)", background: "var(--globe-space)" }}>
        ...
        {GlobeComp && size.w > 0 ? (
          <>
            <GlobeComp
              ...
              backgroundColor={isLight ? "#EDEFF3" : "#080F22"}
            ...
            {/* لایهٔ «لحظه‌ها» — تصویرِ کوچکِ پست‌های دارای موقعیت روی کره */}
            {showMoments && (
              <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 21, overflow: "hidden" }}>
                {moments.map((m) => (
                  <div
                    key={m.id}
                    ref={(el) => {
                      if (el) momentNodes.current.set(m.id, el);
                      else momentNodes.current.delete(m.id);
                    }}
                    style={{ position: "absolute", left: 0, top: 0, display: "none", willChange: "transform" }}
                  >
                    <div
                      onClick={(e) => {
                        ...
                        if (g) {
                          const c = g.controls?.();
                          if (c) c.autoRotate = false;
                          g.pointOfView({ lat: m.lat, lng: m.lng, altitude: 0.12 }, 1200);
                        }
                      }}
                      style={{
                        position: "absolute", left: 0, top: 0,
                    ...
                    >
                      <div
                        style={{
                          ...
                          boxShadow: "0 0 10px #F43F5Ecc, 0 0 3px #F43F5E",
                          background: "#F43F5E22",
                        }}
                      >
                        {m.media_type === "video" ? (
                      ...
                      <div
                        style={{
                          width: 3, height: 20, marginTop: -1, borderRadius: 2,
                          background: "linear-gradient(to bottom, #F43F5E, #F43F5E00)",
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ background: "var(--globe-space)" }}>
            ...
            <p className="text-blue-800 text-xs mt-1">{t("earth.loading")}</p>
          </div>
        )}
      </div>
      ...
      <div className="absolute top-0 inset-x-0 p-3 pointer-events-none" style={{ zIndex: 30 }}>
        <div className="flex items-center gap-2 pointer-events-auto">
          ...
          <button
            ...
            title={showMoments ? t("earth.btn.hideMoments") : t("earth.btn.showMoments")}
            style={{
              background:     showMoments ? "#F43F5Ecc" : "var(--globe-panel)",
              border:         "1px solid " + (showMoments ? "#F43F5E" : "rgba(100,150,255,0.18)"),
              backdropFilter: "blur(18px)",
              color:          showMoments ? "#fff" : "#93c5fd",
            }}
          >
            <ImageIcon size={18} />
          </button>
          <button
            ...
            className="flex-shrink-0 p-2.5 rounded-xl transition-all"
            style={{
              background:     showFilter ? "#6366f1cc" : "var(--globe-panel)",
              border:         "1px solid rgba(100,150,255,0.18)",
              backdropFilter: "blur(18px)",
          ...
          <div className="flex gap-2 mt-2 overflow-x-auto pb-1 no-scrollbar pointer-events-auto">
            {ROLES.map(r => (
              <button
                ...
                style={{
                  background:     roleFilter === r ? "#6366f1cc" : "var(--globe-panel)",
                  border:         "1px solid " + (roleFilter === r ? "#818cf8" : "rgba(100,150,255,0.18)"),
                  color:          roleFilter === r ? "#fff" : "#93c5fd",
                  backdropFilter: "blur(12px)",
                }}
              >
                {r === "همه" ? `${t("earth.role.all")} (${toPersianNum(userCount)})` : `${rem(r)} ${t(rl(r))}`}
              </button>
            ))}
          </div>
        )}
      ...
      {gpsOff && (geoConsent === "permanent" || geoConsent === "temporary") && (
        <div className="absolute inset-x-0 flex justify-center px-3 pointer-events-none" style={{ top: 60, zIndex: 35 }}>
          ...
          >
            ...
            <span className="text-xs text-white">{t("earth.gps.off")}</span>
            <button
              onClick={retryGps}
      ...
      {geoConsent === "ask" && (
        <div
          ...
          style={{ zIndex: 60, background: "var(--globe-overlay)", backdropFilter: "blur(6px)" }}
        >
          <div
            className="w-full max-w-sm rounded-2xl p-5 shadow-2xl"
            dir="rtl"
            style={{ background: "var(--globe-card)", border: "1px solid rgba(100,150,255,0.2)", animation: "slideUp 0.24s ease-out" }}
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-white font-bold text-base">{t("earth.consent.title")}</p>
            </div>
            <p className="text-[13px] leading-6 mb-4" style={{ color: "rgba(147,197,253,0.82)" }}>
              {t("earth.consent.body")}
            </p>
      ...
      {geoBlocked && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-2xl p-5 shadow-2xl"
            dir="rtl"
            style={{ background: "var(--globe-card)", border: "1px solid rgba(255,120,120,0.25)", animation: "slideUp 0.24s ease-out" }}
          >
            <div className="flex items-center gap-3 mb-3">
              <div
                className="w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0"
              ...
              <p className="text-white font-bold text-base">{t("earth.blocked.title")}</p>
            </div>
            <p className="text-[13px] leading-6 mb-3" style={{ color: "rgba(147,197,253,0.82)" }}>
              {t("earth.blocked.body")}
            </p>
            <ol className="text-[13px] leading-7 mb-4 pr-4 list-decimal" style={{ color: "rgba(203,213,225,0.9)" }}>
              <li>{t("earth.blocked.s1Pre")}<b>dilix.ir</b>{t("earth.blocked.s1Suf")}</li>
              <li>{t("earth.blocked.s2")}</li>
              <li>{t("earth.blocked.s3Pre")}<b>{t("earth.blocked.s3Bold")}</b>{t("earth.blocked.s3Suf")}</li>
              <li>{t("earth.blocked.s4")}</li>
            </ol>
            <div className="space-y-2">
              <button
            ...
            {drawer.type === "user" ? (
              ...
              <div className="px-4 pb-4 pt-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        title={drawer.user.online ? t("earth.online") : t("earth.offline")}
                      />
                    </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <MessageCircle size={15} /> {t("earth.btn.chat")}
                  </button>
                  <button
                  ...
                  >
                    <Handshake size={15} /> {t("earth.btn.collaborate")}
                  </button>
                </div>
              </div>
            ) : (
              ...
              <div className="px-3 pb-4 pt-1">
                <div className="flex items-center gap-2 px-1 mb-3">
                  ...
                  <p className="text-sm font-semibold text-white">
                    {toPersianNum(drawer.cluster.count)}{t("earth.cluster.inAreaSuf")}
                  </p>
                </div>
                  ...
                  {drawer.cluster.members.map(u => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px]" style={{ color: rc(u.role) }}>{t(rl(u.role))}</p>
                      </div>
      ...
      {/* ── درگاهِ «لحظه» (پستِ دارای موقعیت) ── */}
      {momentView && (
        <div
          className="absolute inset-x-0 pointer-events-auto"
          style={{ bottom: "60px", zIndex: 45, animation: "slideUp 0.22s ease-out" }}
        >
          <div
            className="mx-3 rounded-2xl shadow-2xl overflow-hidden"
            style={{
          ...
          >
            <div className="flex items-center justify-between px-4 pt-3 pb-1">
              <div style={{ width: 32 }} />
              <div className="w-10 h-1 rounded-full" style={{ background: "rgba(255,255,255,0.15)" }} />
              <button onClick={() => setMomentView(null)} className="p-1" style={{ color: "rgba(147,197,253,0.4)" }}>
                <X size={18} />
              </button>
            </div>
            <div className="px-4 pb-4 pt-1" dir="rtl">
              <button
                type="button"
                onClick={() => router.push(`/u/${momentView.author_earth_id}`)}
                className="flex items-center gap-2.5 mb-3 active:scale-[0.99] transition-transform"
              >
                <div
                  className="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0 border"
                  style={{ borderColor: "rgba(244,63,94,0.5)", background: "rgba(244,63,94,0.15)" }}
                >
                  {momentView.author_avatar
                    ? <img src={momentView.author_avatar} className="w-full h-full object-cover" alt="" />
                    : <span className="text-base">👤</span>}
                </div>
                <div className="min-w-0 text-right">
                  <p className="text-white text-sm font-bold truncate hover:underline">{momentView.author_name}</p>
                  {momentView.place_name && (
                    <p className="text-[11px] flex items-center gap-1" style={{ color: "rgba(244,63,94,0.85)" }}>
                      <MapPin size={11} /> {momentView.place_name}
                    </p>
                  )}
                </div>
              </button>
              <div className="rounded-xl overflow-hidden mb-3" style={{ background: "#000", maxHeight: "42vh" }}>
                {momentView.media_type === "video" ? (
                  <video src={momentView.media_url} controls playsInline
                         className="w-full" style={{ maxHeight: "42vh", objectFit: "contain" }} />
                ) : (
                  <img src={momentView.media_url} alt="" referrerPolicy="no-referrer"
                       className="w-full" style={{ maxHeight: "42vh", objectFit: "contain" }} />
                )}
              </div>
              {momentView.caption && (
                <p className="text-[13px] leading-6 mb-3" style={{ color: "rgba(203,213,225,0.9)" }}>
                  {momentView.caption}
                </p>
              )}
              <button
                onClick={() => router.push(`/u/${momentView.author_earth_id}`)}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-white text-sm font-bold active:scale-95 transition-all"
                style={{ background: "linear-gradient(135deg,#F43F5E,#E11D48)" }}
              >
                <Users size={15} /> {t("earth.btn.viewProfile")}
              </button>
            </div>
          </div>
        </div>
