...
const SERVICES: Service[] = [
  ...
  },
  {
    ...
    href: "/insurance",
    icon: Shield,
    label: "svc.insurance.l",
    description: "svc.insurance.d",
  ...
  },
  {
    ...
    href: "/business",
    icon: BarChart3,
    label: "svc.business.l",
    description: "svc.business.d",
  ...
  },
  {
    ...
    href: "/messages",
    icon: Package,
    label: "svc.messages.l",
    description: "svc.messages.d",
    color: "text-blue-400",
    bg: "bg-blue-500/8",
  ...
  },
  {
    ...
    href: "#",
    icon: FileText,
    label: "svc.contracts.l",
    description: "svc.contracts.d",
  ...
  return (
    <AppShell title={t("services.title")}>
      <div className="page-inner">
          ...
          <Link href="/profile">
            <div className="flex items-center gap-3 p-3.5 rounded-xl bg-[#1C1C1E] border border-white/8 mb-5 hover:border-indigo-500/30 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white">{t("services.kyc.level")} {kycLevel}</p>
                <p className="text-xs text-white/40">{t("services.kyc.desc")}</p>
              </div>
        ...
        {/* Active services */}
        <p className="text-xs text-white/30 font-medium mb-3 pr-1">{t("services.active")}</p>
              ...
              <div className={`flex items-center gap-4 p-4 rounded-2xl border transition-all hover:scale-[1.005] active:scale-[0.995] ${svc.bg} ${svc.border}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="font-semibold text-white text-sm">{t(svc.label)}</p>
                    {svc.badge && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                        svc.badge === "live" ? "bg-emerald-500/15 text-emerald-400" :
                        svc.badge === "new" ? "bg-blue-500/15 text-blue-400" :
                  ...
                  <p className="text-xs text-white/45 leading-relaxed">{t(svc.description)}</p>
                </div>
        ...
        {/* Coming soon */}
        <p className="text-xs text-white/30 font-medium mb-3 pr-1">{t("services.soon")}</p>
        <div className="grid grid-cols-2 gap-2.5">
          {locked.map((svc) => (
            <div key={svc.id} className={`p-4 rounded-2xl border ${svc.bg} ${svc.border} opacity-50`}>
              ...
              <p className="font-semibold text-white/80 text-sm">{t(svc.label)}</p>
              <p className="text-[11px] text-white/35 mt-1 leading-relaxed">{t(svc.description)}</p>
            </div>
          ))}
        </div>
      </div>
