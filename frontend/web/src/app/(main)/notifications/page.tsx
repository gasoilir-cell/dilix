...
function relativeTime(iso: string, t: (k: string) => string): string {
  ...
  if (m < 60) return `${toPersianNum(m)} ${t("notif.time.min")}`;
  if (h < 24) return `${toPersianNum(h)} ${t("notif.time.hour")}`;
  return `${toPersianNum(d)} ${t("notif.time.day")}`;
}
...
export default function NotificationsPage() {
  ...
  // /*rt-poll*/ پولِ اعلان‌ها هر ۱۵ث تا اعلانِ جدید بدونِ رفرشِ دستی دیده شود
  useEffect(() => {
      ...
      <div className="page-inner pb-safe">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-surface-400 text-sm">{t("notif.empty")}</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-surface-600 mt-1">{relativeTime(item.created_at, t)}</p>
                  </div>
                </button>
              );
            })}
